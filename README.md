...
# \- \*\*TenantContext:\*\* Uses `ThreadLocal` to store the active `tenant\_id` per request.  

# \- \*\*Hibernate Filters:\*\* Automatically injects `WHERE tenant\_id = ?` into all SQL queries using \*\*Aspect-Oriented Programming (AOP)\*\*.
...
# Create a .env file based on .env.example with your JWT\_SECRET and DB\_PASSWORD.
